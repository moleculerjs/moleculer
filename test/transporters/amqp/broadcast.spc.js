...
function createNode(name, disableBalancer = false) {
  const broker = new ServiceBroker({
    ...
    //logger: console,
    //logLevel: "error",
    transporter: process.env.AMQP_URI || 'amqp://admin:admin@localhost:5672',
    disableBalancer
  })
  ...
  broker.createService({
    ...
    events: {
      'hello.world'(payload) {
        ...
        if (payload.testing === true) FLOW.push(this.name)
      }
    }
  })
...
describe('Test AMQPTransporter', () => {
  ...
  describe('Test AMQPTransporter event broadcast with built-in balancer', () => {
    ...
    beforeEach(() => {
      ...
      return Promise.all([pub.start(), sub1.start(), sub2.start()]).delay(500)
    })
    ...
    it('Should send an event to all subscribed nodes.', () => {
      ...
      return Promise.delay(2000)
        ...
        .then(() => {
          ...
          expect(FLOW).toEqual(expect.arrayContaining(['pub', 'sub1', 'sub2']))
        })
    }, 10000)
  })
  ...
  describe('Test AMQPTransporter event broadcastLocal with built-in balancer', () => {
    ...
    beforeEach(() => {
      ...
      return Promise.all([pub.start(), sub1.start(), sub2.start()])
    })

    afterEach(() => Promise.all([pub.stop(), sub1.stop(), sub2.stop(), sub3.stop()]))

    it('Should send an event to local services.', () => {
      for (let i = 0; i < 3; i++) pub.broadcastLocal('hello.world', { testing: true })

      return Promise.delay(2000)
        .catch(protectReject)
        .then(() => {
          expect(FLOW).toEqual(['pub', 'pub', 'pub'])
        })
    }, 10000)
  })
})
