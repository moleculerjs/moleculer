...
function work() {
	...
	const p = broker
		...
		.then(() => {
			...
			sumTime += dur;
			if (maxTime == null || maxTime < dur) maxTime = dur;
		})
		.catch(err => {
	...
	// Overload
	if (flood > 0 && broker.transit.pendingRequests.size < flood) setImmediate(work);
	else p.then(() => setImmediate(work));
}
			...
			setInterval(() => {
				if (callCount > 0) {
					...
					console.log(
						broker.nodeID,
						":",
						padStart(Number(rps.toFixed(0)).toLocaleString(), 8),
						"req/s",
						"  Q:",
						color(
						...
						"  E:",
						color(Number(errorCount.toFixed(0)).toLocaleString(), 5, errorCount, 0, 1),
						"  L:",
						color(humanize(latency), 6, latency, 500, 5000),
						"  ML:",
						color(humanize(maxTime), 6, maxTime, 1000, 5000)
					);
					callCount = 0;
					sumTime = 0;
					maxTime = 0;
